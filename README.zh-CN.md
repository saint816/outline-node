...
节点代码块必须填写标题，但创建入口始终可用：输入 `/code` 或空行输入围栏后回车即可创建；若尚无标题，光标会留在代码块上方始终可见的“标题必填”行。也可以直接输入“解析 YAML /code”。已有空标题文件同样显示可编辑提示，但不会擅自改写 Markdown。`/` 菜单只提供 Code 与 Numbered List，已有任务节点及完成状态操作不受影响。
