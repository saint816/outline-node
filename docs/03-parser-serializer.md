...
1. **note 续行**：当前行紧跟在某列表项（或其 note）之后，且该行非空、有前导空白、`前导空白宽度 ≥ 该列表项的内容列`，且本身不匹配列表项正则 → 归为该列表项的 note 行。
   - **例外——note 内未闭合围栏中的空行也是续行**：代码正文里空行极常见，若按普通空行终止 note，`- 节点` + 缩进围栏代码块会在重新解析时从空行处截断，模型被拆成两半（文件字节不丢，但 UI 上代码块断成两截）。parser 因此跟踪 note 内的围栏状态（`ListContext.noteFence`），未闭合期间空行照收。
